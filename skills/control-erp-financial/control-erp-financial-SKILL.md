...
See `references/financial-analysis.md` for full AR detail queries with customer breakdown, single-customer drill-down, and customer summary. Key patterns: SaleDate for aging (NEVER DueDate), WIP/Built in separate bucket, HasCreditAccount marked with asterisk (*).
...
See `references/financial-analysis.md` for full AP detail queries with vendor breakdown, vendor summary, and payment history. Key patterns: DueDate for aging (opposite of AR), ClassTypeID 20009 for bill payments, VendorCreditBalance for credit tracking.
...
See `references/financial-analysis.md` for P&L comparison queries (YoY, MoM) and product-line margin analysis. Key patterns: SUM(-Amount) for revenue, SUM(Amount) for COGS/OpEx, YTD default, COGS is aggregate (not per-product). Product-line margin uses two queries: Query A (revenue breakdown) + Query B (aggregate COGS).
...
See `references/financial-analysis.md` for bank balance, undeposited funds, cash flow summary, and monthly time series queries. Key patterns: bank balance is cumulative (no date filter), cash flow is period-based (YTD default), exclude ZoomTex (10522-10525), track only bank account movements to avoid double-counting. Payment forecast is deferred to analytics milestone -- offer historical cash flow instead.
...
*Financial queries validated against FLS Banners' AR report ($80,899), AP report ($125,319), GL account registers, and Trial Balance export (January 2026). All NodeIDs confirmed from GL SQL Export and modified Trial Balance. AR/AP detail, P&L analysis (YoY/MoM/product-line), and cash flow queries in references/financial-analysis.md (2026-02-09).*
