...
**CRITICAL:** Per CONTEXT.md: "Never guess. If a query falls outside covered domains, say it's not covered."
  ...
  - Count total routing entries (pipe-delimited rows in ROUTING TABLE section, excluding header/separator rows): must be >= 75
...
**Process:**
1. Read the completed routing table from the glossary file
2. For each of the 25 test queries, find the matching routing entry (exact match or closest pattern match)
3. Record PASS or FAIL for each query with the actual vs expected route
4. If any query would be misrouted (routes to wrong skill), fix the routing table entry immediately
5. Re-validate any fixed entries
6. Count final PASS results: must be >= 23/25 (92%) for 90%+ accuracy threshold

**Tracking format (do NOT write to a file, just track internally during verification):**
```
Query 1: "whats our revenue this year" → expected: sales, actual: sales → PASS
Query 2: "show me feather flag sales" → expected: sales, actual: sales → PASS
...
TOTAL: N/25 PASS (X% accuracy)
```
...
**IMPORTANT:** If accuracy is below 23/25 (92%), do NOT mark this task as done. Fix the routing table entries causing failures and re-validate until the threshold is met.
  </action>
  <verify>
