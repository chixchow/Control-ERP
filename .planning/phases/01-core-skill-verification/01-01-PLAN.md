...
must_haves:
  truths:
    ...
    - "Types 3 (Recurring Order), 4 (Credit Memo), and 10 (Vendor Credit Memo) appear in the core skill with their wiki-sourced StatusIDs and a note that they have 0 records at FLS"
    - "The knowledge log Section 7 has no stale TODO markers -- StatusID 2=Built and 4=Closed are filled in, not question marks"
    - "MEMORY.md no longer says 'Project is NOT a git repo' and its StatusID listings match the core skill"
...
<task type="auto">
  ...
  <verify>
    ...
    - `grep -c "28=Closed" skills/control-erp-core/control-erp-core-SKILL.md` returns at least 1 (in TransactionType table)
    - `grep "Type 10" skills/control-erp-core/control-erp-core-SKILL.md` returns a match (Vendor Credit Memo row exists)
    - `grep -c "UNUSED" skills/control-erp-core/control-erp-core-SKILL.md` returns exactly 1 (only Type 5)
    - `grep "3=Sales" skills/control-erp-core/control-erp-core-SKILL.md` returns 0 (all standardized to "Sale")
    - `grep "SaleDate IS NOT NULL" skills/control-erp-core/control-erp-core-SKILL.md` returns at least 2 matches (revenue formula + standard filters note)
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    2. **Fix Section 1 StatusText labels (line ~25):** In the Type 1 row's Notes column, change "StatusIDs: 1=New, 3=Sold, 4=Completed, 9=Voided" to "StatusIDs: 0=New, 1=WIP, 2=Built, 3=Sale, 4=Closed, 9=Voided" to match Section 2 and the core skill.
  ...
  <done>
    Knowledge log Section 7 has resolved StatusIDs (0=New, 1=WIP, 2=Built, 3=Sale, 4=Closed, 9=Voided) with no stale TODO markers. Section 1 StatusText labels match Section 2 and the core skill. Entry template exists for future use. Stub sections 10 and 11 have explicit deferral notes pointing to Phase 3.
  </done>
</task>
