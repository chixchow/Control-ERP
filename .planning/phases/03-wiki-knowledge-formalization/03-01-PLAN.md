...
Do NOT re-run the crawl. Do NOT modify existing wiki files. This is a verification audit.
  ...
  <verify>
Run: `python3 -c "import json; s=json.load(open('output/wiki/_crawl_state.json')); print(len([k for k in s.get('downloaded',s) if isinstance(s,dict)]))"` to confirm entry count.
