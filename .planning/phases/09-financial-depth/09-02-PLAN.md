...
must_haves:
  truths:
    ...
    - "User gets year-to-date results when asking for P&L or cash flow without specifying a date range"
    - "User asking about product-line margin sees a note explaining COGS is aggregate and cannot be split by product at the GL level"
    - "User asking for 'payment forecast' or 'projected cash flow' is told this is not yet available (historical cash flow only)"
  artifacts:
  ...
  key_links:
    ...
    - from: "Product-line revenue query"
      to: "GL + GLAccount"
      via: "GLClassificationType = 4000 grouped by AccountName"
      pattern: "ga.GLClassificationType = 4000"
    - from: "Aggregate COGS query"
      to: "GL + GLAccount"
      via: "GLClassificationType = 5001 summed (separate from revenue query)"
      pattern: "ga.GLClassificationType = 5001"
---
...
Note: Payment forecast / projected cash flow is explicitly deferred to the analytics milestone. This plan covers historical cash flow only.
</objective>
...
3. **Product-Line Margin Analysis** -- This requires TWO separate queries because revenue is per-product at the GL level but COGS is aggregate. Present them as a paired set:
...
Explicitly label these as "Query A" and "Query B" and explain that the user should run both to get the full product-line margin picture: revenue per product line from Query A, total COGS from Query B, then compute gross margin as (Total Revenue - Total COGS).
...
6. Update the NL interpretation table with P&L routing entries:
  ...
  - "P&L by product line" / "margin by product" / "product profitability" -> Product-Line Margin Analysis (both queries)
...
2. Four P&L query blocks present: (a) YoY comparison, (b) MoM comparison, (c) product-line revenue breakdown, (d) aggregate COGS
3. Product-Line Margin explicitly labeled as two separate queries (Query A revenue, Query B COGS)
4. All queries use correct sign conventions (SUM(-Amount) for revenue, SUM(Amount) for COGS/OpEx)
5. COGS limitation note is prominently documented
6. Default date range is YTD
7. NL interpretation table has new P&L routing entries
8. No existing content was deleted or broken
  </verify>
  <done>P&L comparison (YoY and MoM), product-line margin analysis (two-query approach: revenue breakdown + aggregate COGS), and COGS limitation are in the skill file. All queries use correct sign conventions and default to YTD. NL table routes P&L comparison, product profitability, and margin queries correctly.</done>
</task>
...
5. **Cash Flow Notes:**
  ...
  - Payment forecast is deferred to analytics milestone. This skill provides historical cash flow only. If user asks for projected or forecasted cash flow, respond that this capability is planned but not yet available, and offer historical cash flow instead.
...
6. Update the NL interpretation table with cash flow routing entries:
  ...
  - "payment forecast" / "projected cash flow" / "cash forecast" -> NL response: "Payment forecasting is planned for the analytics milestone. Here is the historical cash flow instead:" then route to Cash Flow Summary
...
7. Payment forecast deferral note present in Cash Flow Notes section
8. NL interpretation table has new cash flow routing entries including "payment forecast" redirect
9. No existing content was deleted or broken
  </verify>
  <done>Bank balance, undeposited funds, cash flow summary, and monthly cash flow time series queries are in the skill file. Bank balance is cumulative (no date filter). Cash flow is period-based with YTD default. ZoomTex excluded. Payment forecast explicitly deferred with NL redirect to historical cash flow. NL table routes "bank balance", "cash flow", "cash in and out", and "payment forecast" queries correctly.</done>
</task>
...
The extraction threshold (1,200 lines) is a project decision from STATE.md. This ensures the main skill file stays within Claude's optimal context consumption range.
  ...
  <verify>
Run `wc -l` on the skill file to get line count.

**If <= 1,200 lines (no extraction):**
1. Confirm all new content (AR Detail, AP Detail, P&L Analysis, Cash Flow) is present inline in the skill file
2. Confirm NL interpretation table has all new routing entries
3. Confirm validation line at bottom of file is updated

**If > 1,200 lines (extraction needed):**
1. Confirm `skills/control-erp-financial/references/financial-analysis.md` exists
2. Confirm references file contains all four extracted sections with full query content (search for "AR Detail with Customer Breakdown", "AP Detail with Vendor Breakdown", "P&L Analysis", "CASH FLOW AND BANK BALANCES" headings)
3. Confirm main SKILL.md has brief summary paragraphs for each extracted section with `See references/financial-analysis.md` routing
4. Confirm NL interpretation table is still in the main SKILL.md (not extracted)
5. Confirm no content duplication (queries should be in references/ OR main file, not both)
6. Run `wc -l` on main file again to confirm it is now under 1,200 lines
7. Confirm validation line at bottom of file is updated
  </verify>
  <done>Skill file is at a manageable size. If >1,200 lines, analytical content is extracted to references/financial-analysis.md with summaries in the main file. NL routing remains in the main file regardless of extraction. No content duplication between main file and references file.</done>
</task>
...
2. FIN-07: Bank balance, undeposited funds, cash flow summary, and monthly time series queries are available (historical only; payment forecast explicitly deferred)
...
7. Product-line margin implemented as two queries (revenue breakdown + aggregate COGS) with explicit labeling
8. NL interpretation table is complete with all new routing entries, including payment forecast redirect
9. Skill file is within manageable size (inline or extracted to references/)
10. No existing skill content was modified or removed
</verification>

<success_criteria>
- FIN-06 covered: P&L comparison (YoY, MoM), product-line margin (two-query approach), COGS limitation documented
- FIN-07 covered: Bank balance, undeposited funds, cash flow summary, monthly time series (historical only; payment forecast deferred to analytics milestone with explicit NL redirect)
